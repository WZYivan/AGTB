...
namespace Traverse
{
    ...
    struct TraverseParam
    {
        ...
        std::string ToString() const noexcept
        {
            ...
            std::format_to(sbb, "\n{:-^50}\n", " (X,Y) (m) ");
            std::format_to(sbb, "X = {}, Y = {}\n", x_beg, y_beg);
            std::format_to(sbb, "\n{:-^50}\n", " Begin Azimuth ");
            std::format_to(sbb, "Azimuth = {}\n", azi_beg.ToString());
    ...
    public:
        ClosedLoopSolver(TraverseParam _p)
            : param(std::move(_p)), is_solved(false)
        {
            if (param.angles.size() != param.distances.size())
            {
                AGTB_THROW(Utils::constructor_error, "angles and distances must have same size");
            }
        }
        ...
        bool Solve(int take_precision = 2)
        {
            auto &a = param.angles;
            auto n = a.size();
            ...
            for (auto i = 0; i != n; ++i)
            {
                ...
                va.ddx.at(i) = -va.dx_sum * dis_i / va.dis_sum;
                va.ddy.at(i) = -va.dy_sum * dis_i / va.dis_sum;
